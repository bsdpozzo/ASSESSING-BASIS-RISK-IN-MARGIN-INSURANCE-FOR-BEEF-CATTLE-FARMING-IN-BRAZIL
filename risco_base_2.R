...
modelo_proxy_boi_magro_sp <- lm(vd_bmagro_sp ~ log(vd_bezerro_sp) + vd_milho_bolsa)
options(scipen=9999)
...
ggsave("g3b.svg", plot=g3b, width = 16, height = 9,dpi=300)


##Variaçao da base boi magro MS
modelo_proxy_boi_magro_ms <- lm(log(vd_bmagro_ms) ~ log(vd_bezerro_ms) + log(vd_milho_bolsa))
options(scipen=9999)
...
proxy_boi_magro_previsto_ms <- intercepto_ms + coef_bezerro_ms*log(vd_bezerro_ms)+ coef_milho_ms*log(vd_milho_bolsa)
exp(proxy_boi_magro_previsto_ms)

base_bmagro_ms<-vd_bmagro_ms-exp(proxy_boi_magro_previsto_ms)
mean(base_bmagro_ms)
var(base_bmagro_ms)
sd(base_bmagro_ms)
...
df_ms <- data.frame(
  ...
  Preco_Futuro_MS = as.numeric(exp(proxy_boi_magro_previsto_ms))
)

# Plotando com ggplot
g4b<-ggplot(df_ms, aes(x = Tempo)) +
  geom_line(aes(y = Preco_Spot_MS, color = "Preço Spot Mato Grosso do Sul"), size = 1) +  # Linha dos preços spot
...
ggsave("g4b.svg", plot=g4b, width = 16, height = 9,dpi=300)
...
hist_arima_sp
library(svglite)
ggsave("hist_arima_sp_2.svg", plot=hist_arima_sp, width = 16, height = 9,dpi=300)
install.packages("writexl")  # se ainda não tiver
library(writexl)

write_xlsx(df_erros_mensal_sp, "df_erros_mensal_sp_2.xlsx")
...
# Calcular o preço do boi magro para os dados simulados
proxy_boi_magro_previsto_ms <- intercepto_ms + coef_bezerro_ms*log(preco_bezerro_previsto_ms) + coef_milho_ms*log(p_milho_g)
...
simulacoes_ms_f <- replicate(12, {
  ...
  boi_magro_proxy_sim_ms <- intercepto_ms + coef_bezerro_ms*log(bezerro_sim_ms) + coef_milho_ms*log(preco_milho_sim)
  boi_magro_proxy_sim_ms<-exp(boi_magro_proxy_sim_ms)
...
p_magro_ms<- intercepto_ms + coef_bezerro_ms * log(vd_bezerro_ms) + coef_milho_ms * log(vd_milho_bolsa)
p_magro_ms<-exp(p_magro_ms)
p_magro_g1_ms<-p_magro_ms[117:120]
p_magro_g1_ms<-t(p_magro_g1_ms)
p_magro_g1_ms
proxy_boi_magro_previsto_ms<-exp(proxy_boi_magro_previsto_ms)
